`pof` <- function(
    ...
    result.list <- list()
    incl.cov <- .Call("C_pof", as.matrix(cbind(setms, fuzzyor(setms))), outcome, nec(relation), PACKAGE = "QCAtest")
    ...
    if (is.element("solution.list", names(dots))) {
        ...
        for (i in seq(length.solution)) {
            ...
            incl.cov <- .Call("C_pof", as.matrix(cbind(temp, fuzzyor(temp))), outcome, nec(relation), PACKAGE = "QCAtest")
